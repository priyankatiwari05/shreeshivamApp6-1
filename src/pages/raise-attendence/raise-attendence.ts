...
/**
 * Generated class for the RaiseAttendencePage page.
 *
...
@Component({
  selector: "page-raise-attendence",
  templateUrl: "raise-attendence.html"
})
export class RaiseAttendencePage {
  responseData: any;
  in_time: any;
  out_time: any;
  date: any;
  ...
  raise_attendence_confirm() {
    if (this.in_time == "00:00:00" || this.out_time == "00:00:00") {
      const alert = this.alertCtrl.create({
        title: "Error",
        subTitle: "Please enter IN TIME and OUT TIME both",
        buttons: ["OK"]
      });
      alert.present();
      ...
      if (this.in_time >= this.out_time) {
        const alert = this.alertCtrl.create({
          title: "Error",
          subTitle: "Please enter IN TIME less than OUT TIME",
          buttons: ["OK"]
        });
        alert.present();
      } else {
      ...
      this.authService.postData(data, "raise_attendence").then(
        result => {
          ...
          if (data["status"] == "success") {
            const alert = this.alertCtrl.create({
              title: "Success",
              subTitle: "Attendence request raised successfully",
              buttons: ["OK"]
            });
            loader.dismissAll();
            alert.present();
