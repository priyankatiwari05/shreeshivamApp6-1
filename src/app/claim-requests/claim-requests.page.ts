...
import {NavController, NavParams, AlertController, LoadingController, ModalOptions, ModalController, ToastController} from "@ionic/angular";
...
export class ClaimRequestsPage implements OnInit {
  emp_id:any;
  claims:any;
  page_type: any;
  designation: any;
  ...
  show(index,type)
  {
    ...
    this.navCtrl.navigateForward([ClaimsDetailPage,{claim:myclaim,page_type:'showclaim'}]);
  }

  async presentalert(id,status,status_field,page_title)
  {
    const myModal = await this.modal.create({component:AskHrFormPage,backdropDismiss: true, componentProps:{ page_type:'reimbursement',page_title:page_title}});

    (await myModal).present();
  
    // myModal.onDidDismiss().then((mode,description) => {
    ...
    // myModal.onWillDismiss().then((mode,description) => {
    //   console.log("I'm about to dismiss");
    // });
  }

  async submitstatus(id,status,status_field)
  {
    console.log(id);
    console.log(status);
